...
class ObsidianFormatter(BaseFormatter):
    ...
    def format_lecture(
        ...
        # Gera tag amigável a partir do título do curso
        ...
        # Quebra o texto em parágrafos para melhor leitura
        paragraphs = _split_into_paragraphs(transcript)
        for paragraph in paragraphs:
            lines.append(paragraph)
            lines.append("")

        # Seção de anotações pessoais
        lines.append("---")
        lines.append("")
...
def _split_into_paragraphs(text: str, sentences_per_paragraph: int = 4) -> list[str]:
    ...
    """
    # Se já tem estrutura (timestamps), retorna como está
    if "\n" in text.strip():
        return [text]

    import re

    # Divide em frases (pontuação seguida de espaço)
