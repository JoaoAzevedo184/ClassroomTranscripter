...
Migrado de `udemy_transcripter/setup.py` (v0.1).
Mesmo fluxo: guia o usuário a colar cookies da Udemy e salva com aspas corretas.

Uso:
    classroom-udemy --setup
    # ou
    python -m classroom_transcripter.cli.setup_cli
"""
...
def setup_env() -> None:
    """Cria ou atualiza o arquivo .env interativamente (só cookies Udemy por ora)."""
    env_path = Path(".env")
    ...
    print("    classroom-udemy --url 'https://udemy.com/course/meu-curso/'")
...
def _write_env_file(path: Path, data: dict[str, str]) -> None:
    lines = [
        "# Classroom Transcripter — configuração",
        "# Cookies Udemy: DevTools (F12) → Network → Request Headers → Cookie",
