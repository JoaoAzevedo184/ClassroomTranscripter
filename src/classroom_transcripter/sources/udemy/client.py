...
Usa curl_cffi para imitar o TLS fingerprint do Chrome (necessário pra passar
pela proteção Cloudflare).

Migração v0.1 → v0.2:
  - imports: udemy_transcripter.* → classroom_transcripter.core.*
  - modelos: Section → Module, get_curriculum agora pode devolver Course direto
    (mas mantém get_curriculum() retornando list[Module] pra compat)
  - AuthenticationError/CloudflareBlockError vêm de core.exceptions
"""
...
class UdemyClient:
    ...
    def _setup_auth(self, cookie_data: str) -> None:
        ...
        headers = dict(UDEMY_HEADERS_BASE)
    ...
    def get_course_info(self, slug: str) -> tuple[int, str]:
        ...
        data = self._get(
            f"{UDEMY_API_BASE}/courses/{slug}/",
            params={"fields[course]": "id,title,locale"},
        )
        return data["id"], data["title"]

    def get_curriculum(self, course_id: int) -> list[Module]:
        """Busca toda a grade curricular organizada por módulos (antes: Sections)."""
        modules: list[Module] = []
        current_module = Module(title="Introdução", index=0)

        page_url = f"{UDEMY_API_BASE}/courses/{course_id}/subscriber-curriculum-items/"
        params: dict | None = {
        ...
        while page_url:
            ...
            for item in data.get("results", []):
                ...
                if item_class == "chapter":
                    ...
                    current_module.lectures.append(lecture)
